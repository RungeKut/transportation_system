...
{
	if (GPIO_Pin == HALL_SENSOR_1_Pin)
	{
		BIT_BAND_SRAM(HallState, 0) = BIT_BAND_PER(HALL_SENSOR_1_GPIO_Port->IDR, HALL_SENSOR_1_Pin);
	}
	else if (GPIO_Pin == HALL_SENSOR_2_Pin)
	{
		BIT_BAND_SRAM(HallState, 1) = BIT_BAND_PER(HALL_SENSOR_2_GPIO_Port->IDR, HALL_SENSOR_2_Pin);
	}
	else if (GPIO_Pin == HALL_SENSOR_3_Pin)
	{
		BIT_BAND_SRAM(HallState, 2) = BIT_BAND_PER(HALL_SENSOR_3_GPIO_Port->IDR, HALL_SENSOR_3_Pin);
	}
...
void InitHall(void)
{
	...
	temp =  BIT_BAND_PER(HALL_SENSOR_1_GPIO_Port->IDR, HALL_SENSOR_1_Pin) << 0 |
			BIT_BAND_PER(HALL_SENSOR_2_GPIO_Port->IDR, HALL_SENSOR_2_Pin) << 1 |
			BIT_BAND_PER(HALL_SENSOR_3_GPIO_Port->IDR, HALL_SENSOR_3_Pin) << 2;
...
uint8_t GetPrevHallSector(uint8_t sector)
{
  ...
  if ( (sector > HALL_RESERVED_0_CODE) && (sector < (HALL_RESERVED_7_CODE)) )
  {
    ...
    if ( --sector_offset < 1)
    {// if sector offset is zero then this is reserved state and it should be assigned state before last reserved state
...
uint8_t GetNextHallSector(uint8_t sector)
{
  ...
  if ( (sector > HALL_RESERVED_0_CODE) && (sector < (HALL_RESERVED_7_CODE)) )
  {
    ...
    {// if sector offset is equal last element hen this is reserved state and it should be assigned state next after reserved state
