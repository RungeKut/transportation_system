...
static volatile uint32_t adc_tenso_ready_flag = 0;
static volatile uint8_t CurCh = 0;
static uint8_t decimation_cnt = (1);
static uint16_t TensoADCBuff[(4)];
static uint16_t TensoBuffFiltered[(4)];
static median_context_t tenso_filter_ctx[(4)];
...
static void SelectChannel(uint8_t NewCh)
{
 uint8_t ch = NewCh & ((4) - 1);

 HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000U + 0x00020000U) + 0x0800U)), ((uint16_t)0x4000), (ch & (1<<0)) ? GPIO_PIN_SET : GPIO_PIN_RESET);
 HAL_GPIO_WritePin(((GPIO_TypeDef *) ((0x40000000U + 0x00020000U) + 0x0800U)), ((uint16_t)0x8000), (ch & (1<<1)) ? GPIO_PIN_SET : GPIO_PIN_RESET);
}
# 70 "../Src/adctenso.c"
void ADCTenso_Init(void)
{
...
void ADCTenso_ConvCpltCB(ADC_HandleTypeDef* hadc)
{
 ...
 else
 {
  ...
  (*(volatile uint32_t*)(0x22000000U +32*((uint32_t)((void*)&(adc_tenso_ready_flag))-0x20000000U)+4*((uint32_t)(ch)))) = 1;
  CurCh = (++ch) & ((4) - 1);
  SelectChannel(CurCh);
  decimation_cnt = (1);
 }
...
void ADCTenso_Task(void)
{
 ...
 for (uint8_t i = 0; i < (4); i++)
 {
  if ((*(volatile uint32_t*)(0x22000000U +32*((uint32_t)((void*)&(adc_tenso_ready_flag))-0x20000000U)+4*((uint32_t)(i)))) != 0)
  {
   (*(volatile uint32_t*)(0x22000000U +32*((uint32_t)((void*)&(adc_tenso_ready_flag))-0x20000000U)+4*((uint32_t)(i)))) = 0;
 ...
 *buff_size_ptr = (4) * sizeof(TensoBuffFiltered[0]);
 return 0;
}
