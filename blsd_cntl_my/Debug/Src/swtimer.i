...
# 18 "C:/Keil/blsd_cntl_my/blsd_cntl_my/Inc/swtimer.h" 2
...
# 37 "C:/Keil/blsd_cntl_my/blsd_cntl_my/Inc/swtimer_table.h"
typedef struct
{
...
# 17 "C:/Keil/blsd_cntl_my/blsd_cntl_my/Inc/atomic.h" 2
...
TSwTimer SwTimer_Array[((sizeof(swtimer_size_t)))];
 ...
 uint32_t SwTimer_GetTickCounter(void)
 {
  ...
  if (tick_counter)
  {
   result = tick_counter;
   tick_counter = 0;
   do {__asm volatile("":::"memory");}while(0);
   HAL_SwTimer_Unlock();
   return result;
  }
  else
 ...
 {
  uint8_t i;
  uint32_t tick_cnt = SwTimer_GetTickCounter();
  if (tick_cnt > 0)
  {
   for(i=0; i < ((sizeof(swtimer_size_t))); i++)
   {
...
uint8_t SwTimer_Add(TSwTimerMode mode, uint32_t compare, void (*handler)())
{
 uint8_t i;
...
uint8_t SwTimer_Delete(uint8_t swtimer_id)
{
 if (swtimer_id < ((sizeof(swtimer_size_t))))
 {
